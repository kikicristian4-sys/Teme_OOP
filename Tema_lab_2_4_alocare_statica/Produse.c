...
produs creaza_produs(int id, int cantitate,float pret, const char* model, const char* producator, const char* tip)
{
	...
	rezolvare.id = id;
	rezolvare.pret = pret;
	strcpy_s(rezolvare.model, sizeof(rezolvare.model), model);
	strcpy_s(rezolvare.producator, sizeof(rezolvare.producator), producator);
	strcpy_s(rezolvare.tip, sizeof(rezolvare.tip), tip);
